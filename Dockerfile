...
COPY package.json ./
# Copy package-lock.json if it exists, but don't fail if it doesn't
...
# Note: If package-lock.json is out of sync, npm install will update it
# Using --package-lock-only=false to allow regeneration if needed
RUN if [ -f "package-lock.json" ]; then \
      echo "Using existing package-lock.json"; \
      npm ci --legacy-peer-deps || npm install --legacy-peer-deps; \
    else \
      echo "No package-lock.json found, generating new one"; \
