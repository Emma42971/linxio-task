...
import { ActivityLogPaginationInput, ActivityLogPaginationOutput } from './dto/pagination.dto';
...
export class ActivityLogService {
  ...
  /**
   * Get task activities with cursor-based pagination
   */
  async getTaskActivitiesWithCursor(
    taskId: string,
    pagination: ActivityLogPaginationInput,
  ): Promise<ActivityLogPaginationOutput> {
    // Validate task exists
    const taskExists = await this.prisma.task.findFirst({
      where: { id: taskId },
    });
    ...
    // Add cursor condition for pagination (using ID for cursor)
    if (cursor) {
    ...
    return new ActivityLogPaginationOutput(data, paginationMeta);
  }

  /**
   * Get activities by entity with cursor-based pagination
   */
  async getActivitiesByEntityWithCursor(
    entityType: string,
    entityId: string,
    pagination: ActivityLogPaginationInput,
  ): Promise<ActivityLogPaginationOutput> {
