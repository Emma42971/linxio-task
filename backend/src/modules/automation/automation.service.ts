...
export class AutomationService {
  ...
  async executeRule(
    ...
    try {
      ...
      this.logger.error(
        `Failed to execute rule ${rule.id}`,
  ...
  private checkTriggerConditions(rule: AutomationRule, data: any): boolean {
    // Use the same enhanced condition evaluation as the processor
    const conditions = (rule as any).conditions || rule.triggerConfig?.conditions;
    
    if (!conditions || Object.keys(conditions as object).length === 0) {
      // Legacy condition checks (for backward compatibility)
      return this.evaluateLegacyConditions(rule.triggerConfig, data);
    }

    // Support if/then logic structure
    if (conditions.if) {
      return this.evaluateIfThen(conditions.if, data);
    }

    // Evaluate each condition (AND logic by default)
    for (const [field, condition] of Object.entries(conditions as object)) {
      if (field === 'if' || field === 'then') continue;
